...
public class WindPullArea extends SimpleAbilityItem {


    public WindPullArea(Properties properties) {
    ...
    public InteractionResult useAbilityOnEntity(ItemStack stack, LivingEntity player, LivingEntity interactionTarget, InteractionHand hand) {
        if(!player.level().isClientSide && !interactionTarget.level().isClientSide){
            ...
            pullTarget(player, interactionTarget);
            addCooldown(player);
            useSpirituality(player);
        }
        return InteractionResult.SUCCESS;
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        tooltipComponents.add(Component.literal("Upon use, creates wind that pull all entities in a radius towards you. Can be targeted for the effect to only work on a single entity."));
